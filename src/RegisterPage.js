...
const RegisterPage = () => {
  ...
  // Capture Referral Code and selected course from URL
  useEffect(() => {
    const params = new URLSearchParams(window.location.search);
    const refCode = params.get('ref');
    const courseSlug = params.get('course');
    const selectedCourse = COURSE_LABEL_BY_SLUG[courseSlug] || '';

    if (refCode || selectedCourse) {
      setFormData(prev => ({
        ...prev,
        referredBy: refCode || prev.referredBy,
        selectedCourse: selectedCourse || prev.selectedCourse
      }));
    }
